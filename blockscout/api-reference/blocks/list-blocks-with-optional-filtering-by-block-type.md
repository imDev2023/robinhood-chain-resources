...
````yaml /openapi-specs/Master-pro-api.json get /{chain_id}/api/v2/blocks
openapi: 3.0.0
info:
  contact:
  ...
  /{chain_id}/api/v2/blocks:
    get:
      ...
      description: >
        ...
        When the `type` query parameter is omitted, only main-chain consensus
        blocks

        (equivalent to `type=block`) are returned. Use `type=uncle` to list
        ommer
        ...
        Pagination is cursor-based: the response contains `next_page_params`
        with
