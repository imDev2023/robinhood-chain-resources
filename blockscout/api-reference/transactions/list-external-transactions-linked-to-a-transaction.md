...
````yaml /openapi-specs/Master-pro-api.json get /{chain_id}/api/v2/transactions/{transaction_hash_param}/external-transactions
openapi: 3.0.0
info:
  contact:
...
paths:
  /{chain_id}/api/v2/transactions/{transaction_hash_param}/external-transactions:
    get:
      ...
      description: >-
        Retrieves external transactions that are linked to the specified
        transaction (e.g., Solana transactions in `neon` chain type).
