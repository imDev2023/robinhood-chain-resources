<!-- source: https://developers.uniswap.org/docs/trading/swapping-api/start-building/chained-actions-integration | captured: 2026-09-02 | via: https://developers.uniswap.org/docs/trading/swapping-api/start-building/chained-actions-integration.md (native markdown) -->
# Chained Actions Integration Guide (/docs/trading/swapping-api/start-building/chained-actions-integration)
...
This guide is the operational counterpart to [Swapping with Chained Actions](/docs/trading/swapping-api/concepts/chained-actions). When `/quote` returns `"routing": "CHAINED"`, you drive the trade through the `/plan` endpoints as an ordered sequence of steps under one `planId`.

For conceptual background (plans, steps, the state machine, and routing patterns), read the [concepts page](/docs/trading/swapping-api/concepts/chained-actions). For full request and response schemas, see the [API Reference](/docs/api-reference). AI builders can consume the full OpenAPI Specification at [https://trade-api.gateway.uniswap.org/v1/api.json](https://trade-api.gateway.uniswap.org/v1/api.json).
...
The bridge step is not an AMM swap. It is quoted through the bridge provider (Across Protocol) when the plan is built, so `slippageTolerance` does not apply to it. The bridge runs as a `SEND_TX` step alongside approvals and swaps (see the [execution flow](/docs/trading/swapping-api/concepts/chained-actions#execution-flow)), but only the swap steps have a slippage tolerance band.
...
## API Reference
For full request and response schemas, see the [API Reference](/docs/api-reference). For conceptual background, see [Swapping with Chained Actions](/docs/trading/swapping-api/concepts/chained-actions).
