<!-- source: https://developers.uniswap.org/docs/protocols/v4-hooks/permissioned-pools/provide-liquidity | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v4-hooks/permissioned-pools/provide-liquidity.md (native markdown) -->
# Provide Liquidity (/docs/protocols/v4-hooks/permissioned-pools/provide-liquidity)

Add liquidity to a Uniswap v4 permissioned pool with the Permissioned Position Manager, including minting a position, viewing it, and how issuer recall works.

This guide walks allowlisted liquidity providers through the process of adding liquidity to a Uniswap v4 permissioned pool. It assumes familiarity with the standard Uniswap v4 liquidity provisioning flow and focuses specifically on the technical and operational differences introduced by the permissioned hook. For the deep dive into the contract details, please refer to [Architecture](/docs/protocols/v4-hooks/permissioned-pools/architecture) first.
...
* You use the `PermissionedPositionManager`, not the standard `PositionManager`. See [Deployment Addresses](/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool#deployment-addresses).
...
* The `PermissionedHooks` contract is allowed on the adapter by the adapter admin through `updateAllowedHook`, part of [Step 5 of onboarding](/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool#step-5-approve-virtual-token-contracts). Mints and increases revert with `InvalidHook` otherwise.
...
<img alt="An allowlisted holder mints a non-transferable position NFT; an issuer admin can unwind it, removing the liquidity and returning assets to the holder" src={__img0} placeholder="blur" />

## Where to Go Next
* Understand the contracts in [Architecture](/docs/protocols/v4-hooks/permissioned-pools/architecture).
* Onboard a token in [Deploy a Permissioned Pool](/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool).
* Integrate swaps in an app with [Swapping on Tokenized Pools](/docs/trading/swapping-api/swapping-tokenized-pools).
