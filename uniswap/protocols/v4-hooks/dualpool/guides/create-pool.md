<!-- source: https://developers.uniswap.org/docs/protocols/v4-hooks/dualpool/guides/create-pool | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v4-hooks/dualpool/guides/create-pool.md (native markdown) -->
...
This guide walks the full lifecycle: configuration, initialization, bootstrap, and ongoing operations. It assumes you have a hook to create the pool on; to deploy one, see [Deploy a Hook](/docs/protocols/v4-hooks/dualpool/guides/deploy-hook). One hook serves many pools, so an operator usually deploys once and creates pools as needed.
...
## 2. Choose the Vaults
Each currency may be paired with an [ERC-4626](https://eips.ethereum.org/EIPS/eip-4626) vault that holds the pool's idle capital. Vaults must satisfy, at initialization:
...
See [Liquidity Distributions](/docs/protocols/v4-hooks/dualpool/concepts/liquidity-distributions) for design patterns and validation rules.

[`initializePool`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L292) validates the configuration, binds the vaults, grants their allowances, derives the pool's [virtual-shares offset](/docs/protocols/v4-hooks/dualpool/concepts/lp-shares) from the pair's token decimals, and initializes the v4 pool at `sqrtPriceX96`. Direct `PoolManager.initialize` calls on the hook's pools are blocked.
...
| Function                                                                                                                    | Purpose                                                                                                                    |
| --------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------- |
| [`setPoolLive(key, live)`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L578)              | Pause or resume swaps without changing configuration                                                                       |
| [`setDistribution(key, buckets)`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L490)       | Replace the tick-bucket distribution (blocked mid-JIT-cycle)                                                               |
| [`setExternalDeposits(key, enabled)`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L561)   | Open or close the pool to third-party LPs                                                                                  |
| [`refreshVaultApproval(key, currency)`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L507) | Re-arm a vault's max allowance                                                                                             |
| [`emergencyRevokeVault(key)`](https://github.com/Uniswap/v4-hooks-public/blob/main/src/alf/DualPoolHook.sol#L540)           | Atomically pause the pool, disable deposits, zero both vault allowances, and best-effort drain the vaults back to the hook |

Ownership uses [`Ownable2Step`](https://github.com/OpenZeppelin/openzeppelin-contracts/blob/master/contracts/access/Ownable2Step.sol): transfers require the new owner to accept, and `renounceOwnership` is disabled.
...
## Where to Go Next
* [Provide liquidity to your live pool](/docs/protocols/v4-hooks/dualpool/guides/provide-liquidity)
