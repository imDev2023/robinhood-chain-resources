<!-- source: https://developers.uniswap.org/docs/protocols/the-compact/concepts/core-interfaces | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/the-compact/concepts/core-interfaces.md (native markdown) -->
# Core Interfaces (/docs/protocols/the-compact/concepts/core-interfaces)

Reference key Uniswap The Compact interfaces for deposits, registration, claims, allocator checks, and emissary verification.
...
| ----------------- | -------------- | -------------------------------------------------- |
| `recipient`       | Lower 160 bits | Recipient account address.                         |
| `bytes12 lockTag` | Upper 96 bits  | Destination lock tag used during claim processing. |

This encoding determines processing:

| Outcome             | Condition                                                      |
| ------------------- | -------------------------------------------------------------- |
| **Direct transfer** | `lockTag` matches the claimed lock tag.                        |
| **Convert lock**    | `lockTag` is non-zero and different from the claimed lock tag. |
| **Withdraw**        | `lockTag` is `bytes12(0)`.                                     |
...
| ---------------- | ------------------------------------ |
| `attest`         | `IAllocator.attest.selector`         |
| `authorizeClaim` | `IAllocator.authorizeClaim.selector` |
