<!-- source: https://developers.uniswap.org/docs/protocols/protocol-fee/guides/best-practices | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/protocol-fee/guides/best-practices.md (native markdown) -->
# Best Practices (/docs/protocols/protocol-fee/guides/best-practices)

Apply practical safeguards when integrating with the Uniswap Protocol Fee contracts and release flow.
...
>   In the safe pattern, the caller is responsible for reading the `Firepit.nonce()` offchain, associating it with releasable assets, and passing it as a **calldata** parameter

## Collect Uniswap v3 Fees
For further reading, please see [Read Asset Balances](/docs/protocols/protocol-fee/guides/read-asset-balances)
...
> [!NOTE]
> Integrators can check and collect Uniswap v3 fees before calling `Firepit.release()` so the releasable asset set is up to date.
> 
>   Collection is available via
>   `IV3FeeController.collect()`
> 
>   ```solidity
...
For further reading, please see [Read Asset Balances](/docs/protocols/protocol-fee/guides/read-asset-balances#uniswap-v2-pool-protocol-fees)
