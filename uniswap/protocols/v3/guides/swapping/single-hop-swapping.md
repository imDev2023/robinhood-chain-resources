<!-- source: https://developers.uniswap.org/docs/protocols/v3/guides/swapping/single-hop-swapping | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v3/guides/swapping/single-hop-swapping.md (native markdown) -->
# Single Swaps (/docs/protocols/v3/guides/swapping/single-hop-swapping)

Implement single-hop swaps on Uniswap v3 using exact input and exact output swap patterns.
...
To execute the swap function, we need to populate the `ExactInputSingleParams` with the necessary swap data. These parameters are found in the smart contract interfaces, which can be browsed [here](https://github.com/Uniswap/v3-periphery/blob/main/contracts/interfaces/ISwapRouter.sol).
