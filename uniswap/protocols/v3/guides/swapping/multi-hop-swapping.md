<!-- source: https://developers.uniswap.org/docs/protocols/v3/guides/swapping/multi-hop-swapping | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v3/guides/swapping/multi-hop-swapping.md (native markdown) -->
# Multi-hop Swapping (/docs/protocols/v3/guides/swapping/multi-hop-swapping)

Implement multi-hop swaps on Uniswap v3 using exactInput and exactOutput.

The examples below are implementations of the two styles of multi-hop swapping available on v3. These examples are not production ready code, and are implemented in a simplistic manner for the purpose of learning.
...
* `amountOutMin`: the minimum amount of the outbound asset, less than which will cause the transaction to revert. For the sake of this example we will set it to `0`, in production one will need to use the SDK to quote an expected price, or an onchain price oracle for more advanced manipulation resistant systems.
