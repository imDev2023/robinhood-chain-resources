<!-- source: https://developers.uniswap.org/docs/protocols/v4/concepts/v4-vs-v3 | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v4/concepts/v4-vs-v3.md (native markdown) -->
# v4 vs v3 (/docs/protocols/v4/concepts/v4-vs-v3)

Compare Uniswap v4 and v3 architecture, accounting, and extensibility tradeoffs.
...
and doing multi-hop swaps. Because pools are *contract state* and not entirely new *contracts* themselves, pool creation is significantly cheaper.
...
## Flash accounting
**v4**: The singleton uses *flash accounting*, meaning a caller that unlocks the PoolManager
...
An additional parameter *salt* can be provided when creating liquidity. The
*salt* is used to distinguish positions of the same range on the same pool.
