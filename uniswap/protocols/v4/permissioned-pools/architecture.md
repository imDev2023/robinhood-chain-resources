<!-- source: https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture | captured: 2026-08-22 | via: Jina Reader (native .md endpoint returned 414) -->
# Architecture
...
## [The Permissions Adapter](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#the-permissions-adapter)
...
![Image 1: Creating a virtual version of the token mints the virtual token to the PoolManager; on exit the virtual token is burned and the underlying token is released](https://cdn.developers.uniswap.org/assets/permissioned-pools-virtual-token--VMXu47s.png)
...
## [Permissioned Hooks](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#permissioned-hooks)
...
## [Permissioned Position Manager](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#permissioned-position-manager)
...
![Image 2: An allowlisted holder mints a non-transferable position NFT; an issuer admin can unwind it, removing the liquidity and returning assets to the holder](https://cdn.developers.uniswap.org/assets/permissioned-pools-liquidity-BIrbVYiY.png)
...
## [Swap Routing](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#swap-routing)

Swaps go through the [Universal Router](https://developers.uniswap.org/docs/trading/swapping-api/supported-chains#universal-router-version), which converts permissioned tokens to and from their virtual versions. The router identifies the permissioned token in the swap path, creates a virtual version of it before the pool interaction, executes the swap with the virtual token, and converts back to the original token automatically on exit. The allowlist is enforced twice: by the router when it creates the virtual version of the input token, and by the hook on `beforeSwap`, so the restriction cannot be bypassed.

Permissioned swap routing requires [Universal Router](https://developers.uniswap.org/docs/trading/swapping-api/supported-chains#universal-router-version) 2.2.0 or higher.

![Image 3: Permissioned swap flow: approve, trigger transfer to the Permissions Adapter, create a representation inside Uniswap v4, then settle](https://cdn.developers.uniswap.org/assets/permissioned-pools-swap-flow-Cye936nv.png)

## [Adapter Verification](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#adapter-verification)

A factory deploys and verifies adapters. Verification confirms that an adapter is allowed to hold its underlying token: the issuer seeds the adapter with a minimal balance, as little as 1 wei, which is only possible if the adapter is on the token's allowlist. See [Step 3](https://developers.uniswap.org/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool#step-3-allowlist-and-fund-the-adapter) and [Step 4](https://developers.uniswap.org/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool#step-4-verify-the-adapter) for the mechanics. The factory records a mapping from each adapter to its underlying token, never the reverse.
...
## [Security Considerations](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#security-considerations)
...
## [Where to Go Next](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#where-to-go-next)

*   Onboard a token in [Deploy a Permissioned Pool](https://developers.uniswap.org/docs/protocols/v4-hooks/permissioned-pools/deploy-a-permissioned-pool).
*   Integrate permissioned tokens in an app with [Swapping on Tokenized Pools](https://developers.uniswap.org/docs/trading/swapping-api/swapping-tokenized-pools).
*   Review hook fundamentals in[Hooks](https://developers.uniswap.org/docs/protocols/v4/concepts/hooks).
...
[](https://github.com/uniswap)[](https://discord.gg/uniswap)[](https://twitter.com/Uniswap)

### On this page

[The Permissions Adapter](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#the-permissions-adapter)[Permissioned Hooks](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#permissioned-hooks)[Permissioned Position Manager](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#permissioned-position-manager)[Swap Routing](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#swap-routing)[Adapter Verification](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#adapter-verification)[Security Considerations](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#security-considerations)[Where to Go Next](https://developers.uniswap.org/docs/protocols/v4/permissioned-pools/architecture#where-to-go-next)
