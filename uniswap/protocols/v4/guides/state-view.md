<!-- source: https://developers.uniswap.org/docs/protocols/v4/guides/state-view | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v4/guides/state-view.md (native markdown) -->
# StateView (/docs/protocols/v4/guides/state-view)

Read Uniswap v4 pool state offchain using StateView and compare with onchain StateLibrary usage.
...
If you're familiar with [Reading Pool State](/docs/protocols/v4/guides/read-pool-state), you already know that Uniswap v4 uses **extsload** for efficient data access. For onchain usage, we rely on **StateLibrary** within contracts. However, offchain clients cannot rely on an onchain library for state reads.
...
3. **`getPositionInfo(poolId, positionId)`**
