<!-- source: https://developers.uniswap.org/docs/protocols/v4/guides/hooks/your-first-hook | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/protocols/v4/guides/hooks/your-first-hook.md (native markdown) -->
# Building Your First Hook (/docs/protocols/v4/guides/hooks/your-first-hook)

Build and test a basic points hook in Uniswap v4 using Foundry.
...
Let's start by conceptualizing what we're building today and why. Let's say you have a token named `TOKEN` that you want to incentivize people to buy. One way of doing so is awarding people points when they buy your token. Prior to v4, you'd have to do this offchain or via your own helper contract outside of swap logic, but in v4 you can enable universal access using hooks.
...
*Note: To initiate the swap in the first place, this is where [`UniversalRouter`](/docs/protocols/universal-router/overview) comes into play where we will pass in the [`V4_SWAP`](https://github.com/Uniswap/universal-router/blob/main/contracts/libraries/Commands.sol#L35) command to `UniversalRouter.execute`.*
...
## Basic structure
So far, we’ve created the file named `PointsHook.sol` which only contains the outline of a hook contract. Add this import alongside your other `v4-core` imports (it is required for the hook callback signatures below):

```solidity
import {SwapParams, ModifyLiquidityParams} from "v4-core/src/types/PoolOperation.sol";
```

Let’s add our `afterSwap` and `afterAddLiquidity` hooks to it.
...
## Test suite
The v4-template repo you cloned includes a Foundry harness (`BaseTest`, deploy helpers, and a v4 swap router). Create `test/PointsHook.t.sol` and extend that base test, for example:
...
## Next Steps
Congratulations on building your very first hook! You could explore further by going to [Hook Deployment](/docs/protocols/v4/guides/hooks/hook-deployment) to learn how hook flags work and see how to deploy a hook in action.
