<!-- source: https://developers.uniswap.org/docs/sdks/v4/guides/managing-liquidity/position-minting | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v4/guides/managing-liquidity/position-minting.md (native markdown) -->
# Minting a Position (/docs/sdks/v4/guides/managing-liquidity/position-minting)

Mint a Uniswap v4 liquidity position using SDK position objects and mint call parameters.
...
>
> Contracts that add liquidity at hardcoded extreme tick bounds can be griefed if the pool is not created atomically with the liquidity add. They therefore should not use the extreme ticks, and instead should choose ticks where filling `maxLiquidityPerTick` would be prohibitively expensive. Uniswap's [`InstantLaunchStrategy`](https://github.com/Uniswap/liquidity-launcher/blob/main/src/strategies/InstantLaunchStrategy.sol) does this with its `MIN_LAUNCH_TICK` and `MAX_INITIAL_TICK` values.
...
* It encodes a MINT\_POSITION command with your position parameters (pool key, tickLower, tickUpper, liquidity) and a SETTLE\_PAIR command to pull in the tokens.
