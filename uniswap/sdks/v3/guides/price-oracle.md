<!-- source: https://developers.uniswap.org/docs/sdks/v3/guides/price-oracle | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v3/guides/price-oracle.md (native markdown) -->
# Uniswap as a Price Oracle (/docs/sdks/v3/guides/price-oracle)

Use Uniswap v3 SDK primitives to read and model price oracle data for integrations.

This guide will cover how to fetch price observations from a Uniswap v3 pool to get onchain asset prices.
...
Before diving into this guide, consider reading the theory behind using Uniswap v3 as an [onchain oracle](/docs/protocols/v3/concepts/price-oracles).
...
First, we need to create a Pool contract to fetch data from the blockchain. Check out the [Pool data guide](/docs/sdks/v3/guides/pool-data) to learn how to compute the address and create an **ethers Contract** to interact with.
...
All Uniswap v3 pools store observations of the current tick and the block timestamp.
...
Now that we know the average active Tick over the last 10 blocks, we can calculate the price with the `tickToPrice` function, which returns a `Price` Object. Check out the [Pool data](/docs/sdks/v3/guides/pool-data) guide to understand how to construct a Pool Object and access its properties. We don't need the full Tick Data for this guide.
...
To understand the term **active Liquidity**, check out the [previous guide](/docs/sdks/v3/guides/managing-liquidity/active-liquidity).
...
You can also find an example of a JS multicall in the [Pool data guide](/docs/sdks/v3/guides/pool-data).
...
## Where to Go Next
Now that you are familiar with the Oracle feature of Uniswap, consider checking out the [next guide](/docs/sdks/v3/guides/managing-liquidity/range-orders) on **Range Orders**.
