<!-- source: https://developers.uniswap.org/docs/sdks/v3/guides/managing-liquidity/active-liquidity | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v3/guides/managing-liquidity/active-liquidity.md (native markdown) -->
# Active Liquidity (/docs/sdks/v3/guides/managing-liquidity/active-liquidity)

Track active Uniswap v3 liquidity ranges and position behavior with SDK data models.
...
> [!NOTE]
> > New to this section? Start with [Managing Liquidity Getting Started](/docs/sdks/v3/guides/managing-liquidity/getting-started).

In this guide, we will use the Uniswap v3 subgraph to fetch all ticks from **theGraph** and compute the active liquidity our Pool can use at each Tick. We then use `recharts` to draw a chart that visualizes our Pool's liqudity density.
...
<img alt="LiquidityNet1" src={__img0} placeholder="blur" />
...
To fetch all ticks of our Pool, we will use the [Uniswap v3 graph](/docs/ecosystem/subgraphs/overview).
To visualize active liquidity, we need the **tickIdx**, the **liquidityGross** and the **liquidityNet**.

We define our GraphQL query and [send a POST request](https://axios-http.com/docs/post_example) to the Uniswap v3 subgraph API endpoint:
...
The active liqudity at the current Price is also stored in the smart contract - we already fetched it with the `liquidity` function in the [previous guide](/docs/sdks/v3/guides/pool-data).
...
The `ticks` variable in this code snippet is the result we got from the Uniswap v3 Subgraph earlier.
...
TheUniswap v3 pool here is initialized with only the liquidity of the current Tick.
...
## Where to Go Next
Now that you are familiar with liquidity data, consider checking out our [next guide](/docs/sdks/v3/guides/price-oracle) on using Uniswap as a Price Oracle.
