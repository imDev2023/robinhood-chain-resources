<!-- source: https://developers.uniswap.org/docs/sdks/v3/guides/managing-liquidity/range-orders | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v3/guides/managing-liquidity/range-orders.md (native markdown) -->
# Range Orders (/docs/sdks/v3/guides/managing-liquidity/range-orders)

Create range-order style strategies with Uniswap v3 positions and SDK position math.
...
> [!NOTE]
> This guide builds on top of the [Pooling Liquidity guides](/docs/sdks/v3/guides/managing-liquidity/getting-started).
...
Before working through this guide, consider checking out the Range Orders [concept page](/docs/get-started/concepts/liquidity-providers/range-orders) to understand how Limit orders can be executed with Uniswap v3.
...
If you have read the [Range Order Concept page](/docs/get-started/concepts/liquidity-providers/range-orders), you can skip this section.
...
<img alt="RangeOrder" src={__img0} placeholder="blur" />
...
Our goal for this guide is to create a [Take Profit Order](/docs/get-started/concepts/liquidity-providers/range-orders#take-profit-orders) that trades `token0` for `token1` when the Price of `token0` increases by 5%.
To create our Position, we need to first decide the Tick Range that we want to provide liquidity in.

## Upper tick
We [create a Pool](/docs/sdks/v3/guides/pool-data) that represents the v3 pool we are interacting with and get the `token0Price`.
...
If you are not familiar with liquidity Positions, check out the [liquidity position guides](/docs/sdks/v3/guides/managing-liquidity/getting-started).
...
So if we are using a local fork of mainnet like described in the [Local development guide](/docs/sdks/v3/guides/getting-started), the contract address would be the same as on mainnet.
...
Consider implementing Buy Limit orders as described in the [Range Orders concept page](/docs/get-started/concepts/liquidity-providers/range-orders#buy-limit-orders).
