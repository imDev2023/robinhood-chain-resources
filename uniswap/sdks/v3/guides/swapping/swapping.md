<!-- source: https://developers.uniswap.org/docs/sdks/v3/guides/swapping/swapping | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v3/guides/swapping/swapping.md (native markdown) -->
# Executing a Trade (/docs/sdks/v3/guides/swapping/swapping)

Execute swap flows with the Uniswap v3 SDK by generating calldata and transaction params.

This guide will build off our [quoting guide](/docs/sdks/v3/guides/swapping/quoting) and show how to use a quote to construct and execute a trade on the Uniswap v3 protocol. It is based on the [Trading code example](https://github.com/Uniswap/examples/tree/main/v3-sdk/trading), found in the Uniswap code examples [repository](https://github.com/Uniswap/examples). To run this example, check out the guide's [README](https://github.com/Uniswap/examples/blob/main/v3-sdk/trading/README.md) and follow the setup instructions.

> [!NOTE]
> > New to this section? Start with [Local Development](/docs/sdks/v3/guides/getting-started).
> 
>   To get started with local development, also check out the [local development guide](/docs/sdks/v3/guides/getting-started).
...
To construct our trade, we will first create an model instance of a `Pool`. We create an **ethers** contract like in the [previous guide](/docs/sdks/v3/guides/swapping/quoting#referencing-the-pool-contract-and-fetching-metadata).
...
As shown below, the quote is obtained using the `v3-sdk`'s `SwapQuoter`, in contrast to the [previous quoting guide](/docs/sdks/v3/guides/swapping/quoting), where we directly accessed the smart contact:
...
## Where to Go Next
The resulting example allows for trading between any two ERC20 tokens, but this can be suboptimal for the best pricing and fees. To achieve the best possible price, we use the Uniswap auto router to route through pools to get an optimal cost. Our [routing](/docs/sdks/v3/guides/swapping/routing) guide will show you how to use this router and execute optimal swaps.
