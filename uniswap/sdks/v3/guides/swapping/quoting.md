<!-- source: https://developers.uniswap.org/docs/sdks/v3/guides/swapping/quoting | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v3/guides/swapping/quoting.md (native markdown) -->
# Getting a Quote (/docs/sdks/v3/guides/swapping/quoting)

Get swap quotes with the Uniswap v3 SDK before you build and submit transactions.
...
The default config of the example uses a local fork of mainnet. If you haven't already, check out our [local development guide](/docs/sdks/v3/guides/getting-started).
...
We can find the Pool Factory Contract address for our chain [here](/docs/protocols/v3/deployments).
...
We get the QUOTER\_CONTRACT\_ADDRESS for our chain from [GitHub](https://github.com/Uniswap/v3-periphery/blob/main/deploys.md).

We can now use our Quoter contract to obtain the quote.

In an ideal world, the quoter functions would be `view` functions, which would make them very easy to query onchain with minimal gas costs. However, the Uniswap v3 Quoter contracts rely on state-changing calls designed to be reverted to return the desired data. This means calling the quoter will be very expensive and should not be called onchain.
...
We will dive deeper into routing in the [routing guide](routing).
...
We get the QUOTE\_V2\_CONTRACT\_ADDRESS for our chain from [Uniswap v3 reference deployments](/docs/protocols/v3/deployments).
...
## Where to Go Next
Now that you're able to make a quote, check out our next guide on [swapping](/docs/sdks/v3/guides/swapping/swapping) using this quote!
