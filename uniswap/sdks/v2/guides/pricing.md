<!-- source: https://developers.uniswap.org/docs/sdks/v2/guides/pricing | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/sdks/v2/guides/pricing.md (native markdown) -->
# Pricing (/docs/sdks/v2/guides/pricing)

Calculate mid price and execution price with the Uniswap v2 SDK for swap integrations.
...
You may be wondering why we have to construct a *route* to get the mid price, as opposed to simply getting it from the pair (which, after all, includes all the necessary data). The reason is simple: a route forces us to be opinionated about the *direction* of trading. Routes consist of one or more pairs, an input token and an output token (which fully defines a trading path). In this case, we passed WETH as the input token and DAI as the output token, meaning we're interested in a WETH -> DAI trade.
