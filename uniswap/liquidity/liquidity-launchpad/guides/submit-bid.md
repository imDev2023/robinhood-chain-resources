<!-- source: https://developers.uniswap.org/docs/liquidity/liquidity-launchpad/guides/submit-bid | captured: 2026-08-22 | via: https://developers.uniswap.org/docs/liquidity/liquidity-launchpad/guides/submit-bid.md (native markdown) -->
# Submitting a Bid (/docs/liquidity/liquidity-launchpad/guides/submit-bid)

Submit your first bid in a Uniswap Continuous Clearing Auction and verify execution from onchain logs.
...
The first thing we see happening is the auction creating a new checkpoint for the current block (in this case, block 3). At this point in time the auction has already started (remember how we used `block.number` for the start block?) and you can see that the `cumulativeMps` field, which tracks the percentage of auction which has passed, is at 60\_000, or 6%. We also see that the auction updates the clearing price to the floor price (`79228162514264334008320`). You can verify that this is equal to the expected ratio of 1 ETH to 1 million tokens:
