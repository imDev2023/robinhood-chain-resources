...
| `robinhood-chain/50-stock-token-market-data.md` | Original writing. Which sources answer Stock Token market moments: the issuer's bulk `/rhj/prices` with halt flags and mint and redeem flow (and the fields it zeroes), the now-populated `/rhj/price-deviations`, the 195th token (QNT), the new `GLD / USD` DEX-state Chainlink feed, Dexscreener `tokens/v1` returning only the top pool, viem multicall chunking against free-tier Alchemy, and Nasdaq earnings and split calendars. Measured 2026-09-23. |
| `robinhood-chain/51-anvil-forks-measured.md` | Original writing. Rehearsing on a local anvil fork of chain 4663: anvil's public dev account #0 carries a changing EIP-7702 delegation on mainnet (clear it on the fork), fork gas costs about 20 times mainnet because anvil charges a 1 gwei tip Nitro does not (and `--disable-min-priority-fee` breaks viem's fill path), and viem does not check the RPC's chain id for local keys. Measured 2026-09-24. |
| `robinhood-chain/52-swap-throughput-and-head-following.md` | Original writing. What the chain actually trades and how to follow it: 3,309,659 swaps in 24 hours, 58% v4 and 42% v3-style, 21,419 pools, one v3 WETH/USDG pool taking 11.4% of them, and the 195 tokenized equities at 3%. Both keyed free plans serve `eth_subscribe` for `newHeads` and `logs` over WebSocket although their `eth_getLogs` is capped at 10 and 100 blocks; the public endpoint's WebSocket does not work. With decode CPU, archive size and the gzip ratio, all measured. Measured 2026-09-25. |
| `robinhood-chain/53-launch-sources-measured.md` | Original writing. Where new tokens come from over 24 h: Pons v2 bonding curves (8,852 launches, 0.93% graduate) outnumber all 8,345 new v4 pools, 29% of new v4 pools are hookless 80-99% LP-fee traps, Doppler/Long, Flap, Pools.trade, o1 and a dozen smaller pads with creation topics, buy selectors and anti-snipe schedules, the share of new pools that ever trade, 13 honeypot mechanisms, and public RPC support for state overrides and `eth_simulateV1` (no debug or trace). Measured 2026-09-25. |
