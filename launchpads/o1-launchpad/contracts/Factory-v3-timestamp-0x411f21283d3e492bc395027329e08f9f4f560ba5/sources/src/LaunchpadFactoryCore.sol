...
abstract contract LaunchpadFactoryCore is Ownable2Step, ReentrancyGuard {
    ...
    function createLaunch(LaunchParams calldata p) external payable nonReentrant returns (address token, PoolId poolId) {
    ...
    function _collectCreationFee(address quote, uint256 amount) internal {
    ...
    function registerQuote(address quote, int24 startTickToken0Frame) external onlyOwner nonReentrant {
    ...
    function setQuoteStartTick(address quote, int24 startTickToken0Frame) external onlyOwner nonReentrant {
    ...
    function setQuoteCreationFee(address quote, uint256 creationFee) external onlyOwner nonReentrant {
        if (!quotes[quote].registered) revert QuoteNotRegistered();
    ...
    function unregisterQuote(address quote) external onlyOwner nonReentrant {
        if (!quotes[quote].registered) revert QuoteNotRegistered();
        delete quotes[quote];
        _bumpConfigVersion();
