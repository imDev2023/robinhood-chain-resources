...
///         - on every swap takes a fee on the unspecified currency as an ERC-6909 claim minted to the
///           FeeEscrow, split creator / platform / referrer, plus a block-anchored anti-snipe surcharge
...
contract LaunchHook is BaseHook, Initializable {
    ...
    struct PoolConfig {
        ...
        uint16 antiSnipeStartTotalBps; // total fee at launchBlock (e.g. 9900), decays to baseFeeBps
        uint32 antiSnipeWindowBlocks;
        uint48 launchBlock;
    }
    ...
    function _afterSwap(
        ...
        // v4's afterSwap return delta is denominated in the unspecified currency. On exact-output swaps that
        // is the input, so charging 99% of pool input would only consume about half of the user's gross spend.
        // Fail closed while the anti-snipe surcharge is active; exact-output resumes at the ordinary base fee.
        ...
        uint256 totalFee = _feeAmount(magnitude, totalFeeBps, exactOutput);
        if (totalFee == 0) return (IHooks.afterSwap.selector, int128(0));

        // Take the fee as an ERC-6909 claim minted to the escrow. This offsets the positive hook delta we
        // return below and is balance-safe (no real transfer mid-swap, which a `take` of the input could not
        // guarantee on exact-output swaps).
        poolManager.mint(feeEscrow, feeCurrency.toId(), totalFee);
        _distribute(cfg, feeCurrency, magnitude, totalFee, exactOutput, hookData, key.toId(), sender);

        // forge-lint: disable-next-line(unsafe-typecast) exact-output uses baseFeeBps <= 10%; exact-input < magnitude
    ...
    /// @notice Total fee in bps for the current block: base + a linearly-decaying anti-snipe surcharge over
    ///         the first `antiSnipeWindowBlocks` after launch, capped at `MAX_TOTAL_FEE_BPS`.
    function _totalFeeBps(PoolConfig memory cfg) internal view returns (uint256) {
        uint256 elapsed = block.number - cfg.launchBlock;
        if (elapsed >= cfg.antiSnipeWindowBlocks) return cfg.baseFeeBps;
        uint256 maxSurcharge = uint256(cfg.antiSnipeStartTotalBps) - cfg.baseFeeBps;
        uint256 surcharge = maxSurcharge * (cfg.antiSnipeWindowBlocks - elapsed) / cfg.antiSnipeWindowBlocks;
    ...
    function _validateFeeConfig(PoolConfig calldata cfg) internal pure {
        ...
        if (cfg.antiSnipeWindowBlocks == 0) revert InvalidFeeConfig();
