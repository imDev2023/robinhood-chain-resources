...
abstract contract LaunchpadFactoryCore is Ownable2Step {
    ...
    function createLaunch(LaunchParams calldata p) external returns (address token, PoolId poolId) {
        ...
        if (!quotes[p.quote].registered) revert QuoteNotRegistered();
    ...
    function _poolConfig(bool tokenIsCurrency0) internal view returns (LaunchHook.PoolConfig memory) {
        ...
        return LaunchHook.PoolConfig({
            ...
            antiSnipeWindowBlocks: f.antiSnipeWindowBlocks,
            launchBlock: uint48(block.number)
        });
    }
    ...
    function registerQuote(address quote, int24 startTickToken0Frame) external onlyOwner {
        ...
        quotes[quote] = Quote({registered: true, decimals: dec, startTickToken0Frame: startTickToken0Frame});
        _bumpConfigVersion();
        emit QuoteRegistered(quote, dec, startTickToken0Frame);
    }

    function setQuoteStartTick(address quote, int24 startTickToken0Frame) external onlyOwner {
    ...
    function unregisterQuote(address quote) external onlyOwner {
        if (!quotes[quote].registered) revert QuoteNotRegistered();
        delete quotes[quote];
        _bumpConfigVersion();
    ...
    function setBandTemplate(Band[] calldata newBands) external onlyOwner {
    ...
    function _applyBandTemplate(Band[] memory newBands) internal {
        ...
        for (uint256 i = 0; i < newBands.length; i++) {
            ...
            if (
                newBands[i].lowerOffset % spacing != 0
                    || (!isTail && newBands[i].upperOffset % spacing != 0)
    ...
    function setFeeDefaults(FeeDefaults calldata f) external onlyOwner {
    ...
    function _applyFeeDefaults(FeeDefaults memory f) internal {
        ...
        if (f.antiSnipeWindowBlocks == 0 || f.platformTreasury == address(0)) revert InvalidConfig();
        feeDefaults = f;
        emit FeeDefaultsUpdated(f.platformTreasury, f.baseFeeBps, f.antiSnipeStartTotalBps, f.antiSnipeWindowBlocks);
    }

    function setLaunchSupply(uint256 supply) external onlyOwner {
    ...
    function setAnnouncementRegistry(address registry) external onlyOwner {
    ...
    function setVestingVault(address vault) external onlyOwner {
