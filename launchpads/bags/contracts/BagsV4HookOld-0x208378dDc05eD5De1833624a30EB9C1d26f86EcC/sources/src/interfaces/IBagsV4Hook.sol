...
interface IBagsV4Hook {
    ...
    event PoolRegistered(PoolId indexed poolId, address indexed bondingCurve, address indexed feeShare);
    ...
    /// @param poolId Pool identifier
    /// @param bagsShare Platform share sent to the vault (unwrapped to native)
    /// @param creatorShare Creator share sent to the pool's fee share (as WETH)
    event FeesSwept(PoolId indexed poolId, uint256 bagsShare, uint256 creatorShare);
    ...
    function pools(
        PoolId poolId
    ) external view returns (address bondingCurve, address feeShare, uint128 pendingFees, bool minted);
