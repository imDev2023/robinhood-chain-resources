...
contract BagsV4Hook is BaseHook, Ownable, IBagsV4Hook {
    ...
    /// @notice Bags platform vault (receives 50% of swept fees as native)
    ...
    /// @notice Register a pool's bonding curve and fee share (one-time per pool)
    /// @dev Only callable by the factory. Registration authorizes `bondingCurve` to initialize the
    ///      pool and routes the creator half of swept fees to `feeShare`.
    ...
    function register(
        ...
        emit PoolRegistered(poolId, bondingCurve, feeShare);
    }
    ...
    function sweep(
        ...
        // Split 50/50 between Bags (VAULT) and creator side (feeShare)
        uint256 bagsShare = fees / 2;
        uint256 creatorShare = fees - bagsShare;
        ...
        // Creator share stays as WETH and is distributed by BagsFeeShare
        if (creatorShare > 0) {
            address feeShare = config.feeShare;
            IERC20(WETH).safeTransfer(feeShare, creatorShare);
        ...
        emit FeesSwept(poolId, bagsShare, creatorShare);
    }
