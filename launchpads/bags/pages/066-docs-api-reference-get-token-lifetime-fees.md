...
> Source: https://docs.bags.fm/api-reference/get-token-lifetime-fees
> Retrieved: 2026-09-02 (Mintlify markdown export, curl https://docs.bags.fm/api-reference/get-token-lifetime-fees.md)
...
````yaml GET /token-launch/lifetime-fees
openapi: 3.1.0
info:
...
paths:
  /token-launch/lifetime-fees:
    get:
      tags:
        - Analytics
      summary: Get token lifetime fees
      description: Retrieve the total lifetime fees collected for a specific token
      parameters:
        - name: tokenMint
          in: query
          required: true
      ...
      responses:
        '200':
          description: Successfully retrieved token lifetime fees
          content:
            application/json:
              schema:
                allOf:
                  ...
                  - type: object
                    properties:
                      response:
                        type: string
                        description: >-
                          Total lifetime fees collected in lamports (as string
                          to support bigint casting)
        '400':
          description: Bad request - Invalid token mint or token launch not found
          content:
            application/json:
              schema:
