# Bags - Submit Dexscreener Payment

> Source: https://docs.bags.fm/api-reference/submit-dexscreener-payment
> Retrieved: 2026-09-02 (Mintlify markdown export, curl https://docs.bags.fm/api-reference/submit-dexscreener-payment.md)
...
````yaml POST /solana/dexscreener/submit-payment
openapi: 3.1.0
info:
...
paths:
  /solana/dexscreener/submit-payment:
    post:
      tags:
        - Dexscreener
      summary: Submit Dexscreener payment
      description: Submit the signed payment transaction for a Dexscreener order.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/SubmitDexscreenerPaymentRequest'
      responses:
        '200':
        ...
        '400':
          description: Bad request - Invalid order UUID or payment signature
          content:
            application/json:
              schema:
...
components:
  schemas:
    SubmitDexscreenerPaymentRequest:
      type: object
      properties:
        orderUUID:
          type: string
          minLength: 1
          description: UUID of the order to submit payment for
        paymentSignature:
          type: string
          minLength: 1
          description: Transaction signature of the payment
      required:
        - orderUUID
        - paymentSignature
    SuccessResponse:
