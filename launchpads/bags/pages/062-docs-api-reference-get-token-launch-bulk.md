# Bags - Get Token Launch Bulk

> Source: https://docs.bags.fm/api-reference/get-token-launch-bulk
> Retrieved: 2026-09-02 (Mintlify markdown export, curl https://docs.bags.fm/api-reference/get-token-launch-bulk.md)

---

# Get Token Launch Bulk

> Retrieve token launch records for up to 100 token mints in one request. Results preserve input order, with `null` for a mint that has no launch record.



## OpenAPI

````yaml POST /token-launch/bulk
openapi: 3.1.0
info:
...
paths:
  /token-launch/bulk:
    post:
      tags:
        - Token Launch
      summary: Get token launches in bulk
      description: >-
        Retrieve token launch records for up to 100 token mints in one request.
        Results preserve input order, with `null` for a mint that has no launch
        record.
      requestBody:
        required: true
        content:
            ...
            schema:
              ...
              properties:
                tokenMints:
                  ...
                  maxItems: 100
                  description: >-
                    Base58 public keys of the token mints to look up. Must be
                    unique.
              required:
                - tokenMints
      responses:
        '200':
          description: Successfully retrieved the token launches
          content:
            application/json:
              schema:
                allOf:
                    ...
                    properties:
                      response:
                        ...
                        items:
                          ...
                          nullable: true
                        description: >-
                          Same length and order as `tokenMints`; `null` for a
                          mint with no launch record
        '400':
          description: Bad request - Invalid tokenMints (must be 1-100 unique public keys)
          content:
            application/json:
              schema:
