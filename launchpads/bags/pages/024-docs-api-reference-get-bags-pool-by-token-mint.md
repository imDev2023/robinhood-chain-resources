# Bags - Get Bags Pool by Token Mint

> Source: https://docs.bags.fm/api-reference/get-bags-pool-by-token-mint
> Retrieved: 2026-09-02 (Mintlify markdown export, curl https://docs.bags.fm/api-reference/get-bags-pool-by-token-mint.md)
...
````yaml GET /solana/bags/pools/token-mint
openapi: 3.1.0
info:
...
paths:
  /solana/bags/pools/token-mint:
    get:
      tags:
        - State
      summary: Get Bags pool by token mint
      description: >-
        Retrieve a single Bags pool by its token mint address, with its
        associated Meteora DBC and DAMM v2 pool keys
      parameters:
        - name: tokenMint
          in: query
          required: true
