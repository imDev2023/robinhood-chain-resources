# Bags - Create Partner Config

> Source: https://docs.bags.fm/api-reference/create-partner-configuration
> Retrieved: 2026-09-02 (Mintlify markdown export, curl https://docs.bags.fm/api-reference/create-partner-configuration.md)
...
````yaml POST /fee-share/partner-config/creation-tx
openapi: 3.1.0
info:
...
paths:
  /fee-share/partner-config/creation-tx:
    post:
      tags:
        - Partner
      summary: Get partner config creation transaction
      description: >-
        Get a transaction to create a partner key (partner config) for fee
        sharing. Only one partner key can be created per wallet.
      requestBody:
        required: true
        content:
          application/json:
            schema:
              ...
              properties:
                partnerWallet:
                  type: string
                  description: Public key of the partner wallet that will receive fees
              required:
                - partnerWallet
      responses:
        '200':
          description: Successfully generated partner config creation transaction
          content:
            application/json:
              schema:
                allOf:
                    ...
                    properties:
                      response:
                        ...
                        properties:
                          ...
                          blockhash:
                            $ref: >-
                              #/components/schemas/BlockhashWithExpiryBlockHeight
                        required:
                          - transaction
                          - blockhash
        '400':
          description: Bad request - Invalid wallet or partner config already exists
          content:
            application/json:
              schema:
