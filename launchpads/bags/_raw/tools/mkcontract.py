...
BASE = "/Volumes/Farhan/Work Folder/Dev/AI/long-launch/resources/launchpads/bags"

def build(role, addr, note=""):
    sc = json.load(open(f"{BASE}/_raw/blockscout/smart-contract-{addr}.json"))
    ad = json.load(open(f"{BASE}/_raw/blockscout/address-{addr}.json"))
    ...
    for p, src in files.items():
        ...
        open(fp,"w").write(src)
    if not sc.get("is_verified"):
        import urllib.request
        RPC=os.environ.get("ALCHEMY_MAINNET_URL")
        if RPC:
            req=urllib.request.Request(RPC,data=json.dumps({"jsonrpc":"2.0","id":1,"method":"eth_getCode","params":[addr,"latest"]}).encode(),headers={"content-type":"application/json"})
            code=json.load(urllib.request.urlopen(req)).get("result","0x")
            open(f"{d}/bytecode.hex","w").write(code+"\n")
            L0=[f"Unverified on Blockscout as of 2026-09-02. `bytecode.hex` holds eth_getCode ({len(code)//2-1} bytes)."]
            note=(note+"\n" if note else "")+L0[0]
    ...
    L.append(f"Verified: {sc.get('is_verified')} (verified at {sc.get('verified_at')}).")
