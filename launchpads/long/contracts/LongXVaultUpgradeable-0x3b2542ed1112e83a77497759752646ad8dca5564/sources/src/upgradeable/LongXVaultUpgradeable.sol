...
import {ILighterStateVerifier, IVerifierRouter} from "./ILighterStateVerifier.sol";
...
/// UPGRADEABLE VARIANT (beacon-proxy). Mechanical port of `src/LongXVault.sol`
/// — see that file's history for the audited logic. Differences, in full:
///   - constructor → `initialize` (Initializable; bare implementation is
///     bricked via `_disableInitializers`);
///   - all former immutables are storage, same declaration order;
///   - the verifier is resolved per anchor batch through an IVerifierRouter
///     (`router.verifierFor(batch.batchNumber)`) so Lighter proof-format
///     changes need a router update, not a vault upgrade;
///   - `cfg.admin` is REQUIRED (msg.sender during initialize is the factory)
///     and rotatable via two-step transferAdmin/acceptAdmin;
///   - storage is append-only from here on: new variables go at the END,
///     consuming `__gap` — see src/upgradeable/RUNBOOK.md before upgrading.
contract LongXVaultUpgradeable is Initializable, ERC20Upgradeable, PausableUpgradeable {
    using SafeERC20 for IERC20;

    // ------------------------------------------------------------ config
    IERC20 public usdg; // 6 decimals
    ILighterBridge public bridge;
    IVerifierRouter public router;
    uint16 public assetIndex;
    uint8 public routeType;
    /// @dev The api-key slot the lease installs/parks (v2 lease surface).
    uint8 public apiKeyIndex;
    ...
    uint256 public targetLeverage1e18;
    ...
    uint64 public minOrderNotional6;
    ...
    address public expectedL1Owner;
    ...
    uint64 public challengeBatches;
    ...
    uint64 public executionBatches;
    ...
    uint16 public rescuerBountyBps;
    uint16 public executorBountyBps;
    ...
    struct VaultConfig {
        ...
        address admin; // v5.3.3 ops role; REQUIRED (initialize's msg.sender is the factory, never a person)
    }
    ...
    ///      in `initialize` (an inline initializer would run in the
    ///      implementation's context and leave the proxy at 0).
    ...
    uint16 public bondBps; // bps of proven equity; 0 = no penalty (harness only)

    /// @dev v5.3.3 ops role: raiseCap / pause / unpause / forceUnwind /
    ///      withdrawFees — since v5.4 the ONLY privileged role (the owner
    ///      emergency surface is gone). REQUIRED at initialize; rotatable via
    ///      the two-step transferAdmin/acceptAdmin.
    address public admin;
    /// @dev Two-step admin rotation: the nominee here must call `acceptAdmin`
    ///      (proving control of the address) before the role moves. Cleared on
    ///      acceptance; re-nominate (or nominate address(0)) to cancel.
    ...
    /// @dev The implementation contract is inert: only proxies initialize.
    constructor() {
    ...
    function initialize(
        ...
        __Pausable_init();
        // Under the factory-created BeaconProxy, msg.sender here is the
        // FACTORY — a msg.sender fallback would silently make it the admin,
        // and the router is load-bearing for every proof path.
        if (cfg.admin == address(0)) revert InvalidConfig();
        if (address(cfg.router) == address(0)) revert InvalidConfig();
        ...
        // silently truncated — register a verifier sized for the window. This
        // checks the router's LATEST verifier; the same invariant must hold
        // for every verifier registered later (script-enforced, see RUNBOOK).
        if (cfg.router.latestVerifier().freshnessBatches() < tc) revert InvalidConfig();
        bondBps = cfg.bondBps;
    ...
    function proveState(
        ...
        LighterProof.AccountFacts memory f =
            router.verifierFor(batch.batchNumber).verifyAccount(w, batch, accountIndex, expectedL1Owner, targetMarket);
    ...
    {
        f = router.verifierFor(batch.batchNumber).verifyAccount(w, batch, accountIndex, expectedL1Owner, targetMarket);
    ...
    /// @notice Nominate a new admin (two-step, mirroring Ownable2Step): the
    ///         role only moves when the nominee calls `acceptAdmin`.
    ///         Re-nominate to overwrite a pending offer; nominate address(0)
    ///         to cancel it.
    ...
    /// @notice Complete an admin rotation — callable only by the nominee,
    ///         proving the destination address can transact on this chain
    ///         before the old admin relinquishes anything.
    ...
    /// @dev Reserved slots for future storage appended via upgrades. New
    ///      variables MUST be declared immediately before this gap, shrinking
    ///      it by the slots they consume — never reorder, retype, or remove
    ///      anything above. See src/upgradeable/RUNBOOK.md.
