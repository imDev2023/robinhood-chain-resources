...
Contract name: DopplerHookInitializer.
...
Multicurve pool initializer used by every Long launch; holds the 95/5 LP-fee beneficiary split. Documented in resources/launchpads/doppler/.
...
- `Collect(bytes32,uint256,uint256)`
- `Create(address,address,address)`
- `DelegateAuthority(address,address)`
- `Graduate(address)`
- `Lock(address,tuple[])`
- `ModifyLiquidity(tuple,tuple)`
- `Release(bytes32,address,uint256,uint256)`
- `SetDopplerHook(address,address)`
- `SetDopplerHookState(address,uint256)`
- `Swap(address,tuple,bytes32,tuple,int128,int128,bytes)`
- `UpdateBeneficiary(bytes32,address,address)`
