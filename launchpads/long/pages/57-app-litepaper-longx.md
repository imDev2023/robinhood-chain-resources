# Long - LongX litepaper, Tokenized Cross-Domain Leverage (PDF served at /litepaper)

> Source: https://app.long.xyz/litepaper
> Retrieved: 2026-09-02 (Jina Reader PDF extraction; a direct download of the PDF returned the Cloudflare challenge page instead, so only the extracted text is archived)
> Raw capture: `_raw/jina/app-litepaper.md`

---

Linked from the @longdotxyz post of 2026-08-26 (`socials/01-x-longdotxyz.md`).
This is the only document Long publishes about any of its mechanisms, and it covers LongX (leveraged tokens on Lighter), not the launchpad.
Five pages; formulae are flattened by the text extraction, so read the PDF at the source URL for the maths.
