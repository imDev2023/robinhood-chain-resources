...
contract MultiHookRouter is ERC165, IACPHook, ReentrancyGuardTransient {
    // ──────────────────── Immutables ────────────────────
    ...
    modifier onlyCoreAdmin() {
        if (!AgenticCommerceV3(acpContract).hasRole(AgenticCommerceV3(acpContract).ADMIN_ROLE(), msg.sender))
            revert NotCoreAdmin();
        _;
    }

    modifier onlyJobClient(uint256 jobId) {
        AgenticCommerceV3.Job memory job = AgenticCommerceV3(acpContract).getJob(jobId);
    ...
    modifier hooksNotLocked(uint256 jobId) {
        AgenticCommerceV3.Job memory job = AgenticCommerceV3(acpContract).getJob(jobId);
        if (job.status != AgenticCommerceV3.JobStatus.Open) revert HooksLocked();
        _;
    }
    ...
    constructor(address acpContract_, uint256 maxHooksPerJob_) {
    ...
    function setMaxHooksPerJob(uint256 newMax) external onlyCoreAdmin {
    ...
    ///      FundTransferHook and SubscriptionHook subscribe to overlapping
    ///      selectors (`fund`, `setBudget`) with incompatible optParams ABIs,
    ///      so dispatch mode is mandatory whenever both are configured -- which
    ///      makes order preservation a hard requirement.
    function removeHook(
        uint256 jobId,
        bytes4 selector,
        address hook
    ...
    function beforeAction(
        ...
        for (uint256 i; i < len; ) {
            if (!AgenticCommerceV3(acpContract).whitelistedHooks(hooks[i])) {
    ...
    function getJob(uint256 jobId) external view returns (AgenticCommerceV3.Job memory) {
        return AgenticCommerceV3(acpContract).getJob(jobId);
    }

    /// @notice Passthrough to core paymentToken
    function paymentToken() external view returns (IERC20) {
        return AgenticCommerceV3(acpContract).paymentToken();
    }
    ...
    /// @dev Validate a sub-hook: non-zero, whitelisted on core, supports IACPHook and IACPHookMetadata
    function _validateSubHook(address hook) private view {
        if (hook == address(0)) revert ZeroAddress();
        if (!AgenticCommerceV3(acpContract).whitelistedHooks(hook))
            revert SubHookNotWhitelisted();
        if (!ERC165Checker.supportsInterface(hook, type(IACPHook).interfaceId))
            revert InvalidHook();
        if (!ERC165Checker.supportsInterface(hook, type(IACPHookMetadata).interfaceId))
            revert InvalidHook();
    }
    ...
    function _extractOptParams(
        ...
        if (selector == SEL_SET_BUDGET) {
            (, , bytes memory optParams) = abi.decode(data, (address, uint256, bytes));
            return optParams;
        } else if (selector == SEL_FUND) {
            (, bytes memory optParams) = abi.decode(data, (address, bytes));
            return optParams;
        } else {
            // submit, complete, reject: (address, bytes32, bytes)
    ...
    function _reEncodeData(
        ...
        if (selector == SEL_SET_BUDGET) {
            (address caller, uint256 amount, ) = abi.decode(data, (address, uint256, bytes));
            return abi.encode(caller, amount, hookOptParams);
        } else if (selector == SEL_FUND) {
    ...
    ///      it declares as required via IACPHookMetadata.requiredSelectors().
    ...
    function _validateSelectorCompleteness(uint256 jobId) private view {
        ...
        // Collect unique hooks across all selectors (sum actual stored lengths)
        uint256 maxUnique;
        for (uint256 s; s < 5; ) {
        ...
        for (uint256 h; h < uniqueCount; ) {
            bytes4[] memory required = IACPHookMetadata(uniqueHooks[h]).requiredSelectors();
