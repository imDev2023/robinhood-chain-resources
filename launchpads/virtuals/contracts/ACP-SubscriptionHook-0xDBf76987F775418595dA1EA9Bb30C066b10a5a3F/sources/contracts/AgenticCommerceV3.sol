...
// ERC-ACP: Agentic Commerce V3 — job escrow with evaluator attestation + hook system
...
/**
 * @title AgenticCommerceV3
 * @notice Job escrow protocol with evaluator attestation and an extensible hook system.
 *         Clients post jobs, providers deliver work, evaluators attest quality.
 *         Funds are escrowed on-chain and distributed on completion.
 * @dev State machine: Open -> Funded -> Submitted -> Completed | Rejected | Expired.
 *
 *      Hook system (optional per job):
 ...
 *        When hook == address(0), the contract behaves like vanilla ERC-ACP.
 *
 *      Fee distribution on completion:
 *        platformFee  = budget * platformFeeBP / 10000       -> platformTreasury
 *        evaluatorFee = budget * evaluatorFeeBP / 10000      -> evaluator
 *        net          = budget - platformFee - evaluatorFee  -> provider
 *
 *      Upgradeable via UUPS proxy pattern (OpenZeppelin).
 */
contract AgenticCommerceV3 is Initializable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardTransient, UUPSUpgradeable {
    ...
    /// @notice Core job data, packed into 6 storage slots
    ...
    /// @param evaluator  Third-party attestor (address(0) = auto-complete on submit)
    ...
    struct Job {
        ...
        address hook;        // 20 ──  slot 4
        uint256 budget;      // 32 ──  slot 5
        string description;  //        slot 6+
    }

    /// @notice ERC-20 token used for all job payments and escrow
    ...
    /// @notice Emitted when a job is completed (by evaluator or auto-complete)
    event JobCompleted(
        uint256 indexed jobId,
    ...
    /// @notice Emitted when escrowed funds are returned to the client (reject or expiry)
    event Refunded(
        uint256 indexed jobId,
        address indexed client,
        uint256 amount
    ...
    /// @notice Initializes the proxy with payment token, treasury, and admin
    /// @param paymentToken_ ERC-20 token for all job payments
    /// @param treasury_ Address that receives platform fees
    /// @param admin_ Address granted ADMIN_ROLE and DEFAULT_ADMIN_ROLE
    ...
    /// @notice Emergency withdrawal while the contract is paused.
    ...
    ///         Once detached, the job behaves like vanilla ERC-ACP (no gating, no bookkeeping).
    ...
    function createJob(
        ...
        if (expiredAt <= block.timestamp + 5 minutes || expiredAt > type(uint48).max) revert ExpiryTooShort();
    ...
    function setProvider(uint256 jobId, address provider_) external whenNotPaused {
        ...
        if (msg.sender == provider_) revert ClientIsProvider();
    ...
    ///         or Funded -> Completed (auto-complete when evaluator == address(0)).
    ///         Also accepts Open -> Completed for zero-budget jobs without an evaluator.
    ...
    function submit(
        ...
        if (job.evaluator == address(0)) {
            ...
            // Also fire afterComplete hook so hooks can react to completion.
            // Sentinel: caller is address(0) to signal auto-complete (no evaluator).
            // Hook developers: check caller == address(0) in _postComplete to
            // distinguish auto-complete from evaluator-driven completion.
            _afterHook(
                job.hook,
                jobId,
                this.complete.selector,
                abi.encode(address(0), deliverable, optParams)
            );
        } else {
    ...
    ///         - Funded/Submitted with evaluator: evaluator only
    ///         - Funded/Submitted without evaluator: client or provider
    ...
    function reject(
        ...
        if (job.status == JobStatus.Open) {
            ...
            if (job.evaluator == address(0)) {
                // No evaluator — client or provider can reject
                if (msg.sender != job.client && msg.sender != job.provider) revert Unauthorized();
            } else {
                if (msg.sender != job.evaluator) revert Unauthorized();
            }
    ...
    function claimRefund(uint256 jobId) external whenNotPaused nonReentrant {
        ...
        if (job.status == JobStatus.Submitted) {
            if (block.timestamp < job.expiredAt + EVALUATOR_GRACE_PERIOD) revert WrongStatus();
        } else {
            if (block.timestamp < job.expiredAt) revert WrongStatus();
        }
    ...
    /// @notice Returns the full job data for a given job ID
