...
import {V2BuybackVault} from "../V2BuybackVault.sol";
import {FeePolicySnapshot, IV2FeeEscrow, IV2FeePolicy} from "../interfaces/ILaunchpadV2.sol";

/**
 * @title V2MemeHook
 * @notice Singleton Uniswap V4 hook shared by every graduated launchpad v2 pool.
 ...
 * buyback-and-burn split that governs V2BondingCurve's pre-graduation
 * fee sweep lives here, read live by the curve through IV2FeePolicy so
 * both phases behave identically.
 */
contract V2MemeHook is BaseHook, IUnlockCallback, IV2FeePolicy, Ownable2Step, ReentrancyGuard {
    ...
    struct LaunchInfo {
        ...
        // Creator-chosen at launch on V2LaunchFactory, snapshotted here
    ...
    // Mirrors V2BondingCurve's own ceiling, so a graduated pool can never
    ...
    V2BuybackVault public buybackVault;
     ...
     */
    function setBuybackVault(V2BuybackVault buybackVault_) external onlyOwner {
