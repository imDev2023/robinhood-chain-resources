...
import {IV2FeeEscrow, IV2FeePolicy, IV2LaunchFactory} from "./interfaces/ILaunchpadV2.sol";

/**
 * @title V2BuybackVault
 ...
 * "single deployment for every token" pattern V2FeeEscrow already uses.
 *
 ...
 */
contract V2BuybackVault is Ownable2Step, ReentrancyGuard {
    ...
    IV2FeePolicy public immutable feePolicy;
     ...
     */
    constructor(address initialOwner, IV2FeePolicy feePolicy_, IV2FeeEscrow feeEscrow_) Ownable(initialOwner) {
    ...
    function _isAuthorizedLocker(address token, address caller) private view returns (bool) {
        ...
        return caller == IV2LaunchFactory(factory).getLaunchedToken(token).curve;
    }
}
